...
/// Serialize a tile set to a file with paths stored relative to `base`.
///
/// Makes the on-disk analysis cache (`.emosaic_v2_*`) portable across
/// machines / between local dev and the cloud runner. The file format is
/// bincode of `(Vec<Tile<Vec<u8>>>, Vec<String>)` where the strings are
/// relative paths.
pub fn write_to_file<const N: usize>(
    ts: &TileSet<[Rgb<u8>; N]>,
    path: &Path,
    base: &Path,
) -> std::io::Result<()> {
    let colors: Vec<Tile<Vec<u8>>> = ts
        .tiles
        .iter()
        .map(|tile| {
            tile.clone().map(|rgbs| {
    ...
    let rel_paths: Vec<String> = ts
        ...
        .collect();
    let bytes = bincode::serialize(&(colors, rel_paths))
        .map_err(|e| std::io::Error::new(std::io::ErrorKind::Other, e))?;
    std::fs::write(path, bytes)
}

/// Deserialize a tile set written by `write_to_file`, prepending `base` to
/// each stored relative path to recover absolute paths.
pub fn read_from_file<const N: usize>(
    path: &Path,
    base: &Path,
) -> std::io::Result<TileSet<[Rgb<u8>; N]>> {
    let bytes = std::fs::read(path)?;
    let (colors, rel_paths): (Vec<Tile<Vec<u8>>>, Vec<String>) = bincode::deserialize(&bytes)
        .map_err(|e| std::io::Error::new(std::io::ErrorKind::Other, e))?;
    let tiles: Vec<Tile<[Rgb<u8>; N]>> = colors
        .into_iter()
        .map(|tile| {
            let colors_vec: Vec<Rgb<u8>> = tile
                .colors
                .chunks(3)
                .map(|chunk| Rgb([chunk[0], chunk[1], chunk[2]]))
                .collect();
            let colors_array: [Rgb<u8>; N] = colors_vec.try_into().map_err(|_| {
                std::io::Error::new(std::io::ErrorKind::InvalidData, "tile colors length mismatch")
            })?;
            Ok(Tile {
                colors: colors_array,
                ..tile
    ...
    let abs_paths: Vec<PathBuf> = rel_paths.into_iter().map(|rel| base.join(rel)).collect();
    Ok(TileSet::from_tiles(tiles, abs_paths))
}
